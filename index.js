...
const TOKEN =
  "eyJhbGciOiJIUzUxMiIsInR5cCI6IkpXVCJ9.eyJpcCI6IjE4MS4xNzYuODQuNjgiLCJwbGF0Zm9ybSI6IkFQSSIsInVzdWFyaW8iOnsiX2lkIjoiNjg3N2FkNjk3YjI0MjIwMjY3ZTg0NDgwIiwibmFtZSI6ImZpa29fZG94IiwicmFuZ28iOiJwbGF0aW51bSIsInNwYW0iOjAsImNfZXhwaXJ5IjoxNzgxOTY3MzE0fSwiaWF0IjoxNzc0NjI0ODM1LCJleHAiOjE3ODE4ODI0MzV9.Wtw-ueqZMVbql4fAqd1s6sDCb6vW_T0bBUxOp9BoD5YKgVby8Ptn_0LRmvF00XXM0ztcVdxw7PIqNeWLAFuV0Q";

// Si usas Node < 18, descomenta esto y añade node-fetch en package.json
...
async function consultarAPI(endpoint, body) {
  ...
  if (contentType && contentType.includes("application/json")) {
    ...
    if (!response.ok || data.message)
      throw new Error(data.message || `Error consultando ${endpoint}`);
    return data;
  } else {
...
app.all("/api/nombres", async (req, res) => {
  ...
  if (!ap_pat && !ap_mat && !nombres) {
    return res.status(400).json({
      error:
        "Debes proporcionar al menos un parámetro: ap_pat, ap_mat o nombres.",
    });
  }
...
app.get("/api/sbs", async (req, res) => {
  ...
  try {
    ...
    res.json({
      rcc_padron: data.rcc_padron ?? null,
      rcc_reporte: data.rcc_reporte ?? [],
    });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
app.get("/api/telefonos", async (req, res) => {
  ...
  if (!dni && !numb)
    return res.status(400).json({ error: "Debes proporcionar dni o numb." });
  if (dni && !/^\d{8}$/.test(dni))
    return res
      .status(400)
      .json({ error: "El DNI debe tener 8 dígitos." });
  if (numb && !/^\d{9}$/.test(numb))
    return res
      .status(400)
      .json({ error: "El número debe tener 9 dígitos." });
...
app.get("/api/sunat", async (req, res) => {
  ...
  if (!/^\d{11}$/.test(ruc))
    return res
      .status(400)
      .json({ error: "La RUC debe tener 11 dígitos." });
